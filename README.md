...
```
   TIME   |  TASK    | NOTES
----------------------------------------------------------------------------------------------------------------------
0500 0600 |  예배 1.0 | 5AM [블루콜리] [요거트, 아몬드] [콜라겐, 비타민C 1000mg, 오메가3 1000mg, MSM 1000mg]
0600 0800 |  운동 2.0 | 8AM [프로틴 쉐이크, 강황/차가]
0800 0500 |  일과 8.0 | 4PM [MEAL]
0500 0900 |  자유 4.0 | 5PM [블루콜리] [요거트, 아몬드] [콜라겐, 비타민C 2000mg, 오메가3 1000mg, MSM 1000mg, 아연, 구리]
----------------------------------------------------------------------------------------------------------------------
```
...
-----------------------------------------------------------------------------------------
|      | Pull(100)   Abs         | Push(300)     Abs         |                          |
|      | Curl(100)   Neck        | Band(300)     Neck        |                          |
|      | Aero(1HR)   Aero(1HR)   | Aero(1HR)     Aero(1HR)   | Aero(2HR)    Aero(3HR)   |
-----------------------------------------------------------------------------------------
* Monday
...
* WEEKLY GROCERY
  ...
  - 다진 마늘
  - 올리브
  - 토마토 파스타 소스 2+lbs (32+oz)
  ------------------------------
  - 토띠야
  ------------------------------

* WEEKLY PROTEIN PLAN (상승다이어트 877+g/week; 65g x7day x2 =910)
  <오전 165g>
...
* 감량은 운동량을 늘려라. 식단을 줄이니까 멘탈과 피지컬 컨디션이 너무 안좋다.
 ```
...
```
대화
1. "긍정적 이성적 미래적인 / 내 이야기를 / 낮게 적게 매너있게한다." (말할때 3-1-3)
2. "어, 그래, 진짜를 활용한다."         (들을때)
...
2. 주식 포트폴리오
   ...
   - 20% 중국
     - 10% 심천 (CNXT)
     - 10% 항셍50 (YINN, FXI, CWEB)

3. 아이디어
- 레버리지
   - 미국
      - 버핏지수 TMC/(GDP+Bank)가 90% 이하인 순간부터 2X/3X 레버리지 TQQQ ETF매수를 고려한다. 매수한 ETF는 들고간다.
   - 중국
      - 버핏지수 TMC/(GDP+Bank)가 30% 이하인 순간부터 2X/3X 레버리지 YINN ETF매수를 고려한다. 매수한 ETF는 2년내에 매도한다.
   - "망하는 건 순식간이다" 위의 시나리오가 아니면 레버리지 ETF를 피하고 적립식으로 1배수 ETF를 산다.
- 소비계획
   - 자동차는 총자산의 5% 까지만 (사고 싶은 차의 20배가 있으면 그 차를 산다)
   - 집은 렌트로 살다가 가정을 이루고 자녀가 생겨서 정착하면 구입한다.
- 미래산업
   - 유망산업도 너무 오래전에 들어가면 의미가 없다. 유전자분야도 대폭락장에 들어가자
```
